...
const Signup = () => {
  ...
  const validateInputs = () => {
    ...
    if (!formDate.name) return toast.error('Please enter your name');
    ...
    <div className="content flex items-center justify-between py-10 px-10 max-w-[800px] mx-auto">
      <div className="bg-base-200 border-base-300 rounded-box w-xs border p-4">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-[#3a5365] capitalize text-sm text-center">
            You are a few steps away from communicating better with your friends
          </span>
        </div>
        <form onSubmit={handleSubmit} className="fieldset flex flex-col gap-5">
          <div className="form-group relative">
            <label className="label text-[#3e5769]">Name</label>
            <input
              type="text"
            ...
            />
            <User className="absolute bottom-2 left-2 text-[#745ab4] p-1 z-1" />
          </div>
          <div className="form-group relative">
          ...
          <button className="btn btn-neutral mt-4 bg-[#5d478f] flex items-center gap-3 hover:bg-[#5d478f]/50 hover:border-none hover:text-black">
            signup <SquareArrowOutUpRightIcon size={15} />
          </button>
